...
class Customer_bill(Customer_details):
    ...
    def discount_products(self):
        ...
        while True:
            ...
            if  user_choosen_discount_product in Customer_bill.discount_items:            
                self.customers_items.update([(user_choosen_discount_product,discount_product_quantity)])
    ...
    def total_calculaton(self):
        for i,j in Customer_details.customers_items.items():
            ...
            elif i in Customer_bill.discount_items:
                discount_items_calulation=Customer_bill.discount_items[i]
                items_discount=Customer_bill.discount_items[i]
                items_price=items_discount[1][0]
                discount_percentage_for_choosen_product=Customer_bill.dicount_percentage[i]
                price_for_customer_discount_items=(items_price-((discount_percentage_for_choosen_product/100)*items_price))*j
                Customer_bill.for_bill.append([Customer_bill.s_no,items_discount[0],j,items_price,items_discount[1][1],price_for_customer_discount_items])
                Customer_bill.s_no=Customer_bill.s_no+1
        ...
        print('Thank you')
class luck_draw(Customer_bill):
    def __init__(self,customers_id):
        self.customers_id=customers_id
    def coupon(self):
        print('Fill the Coupon for Lucky Draw')
        while True:
            customer_id_for_lucky=input('Enter Customer ID: ')
            if customer_id_for_lucky in self.customer_details:
                customer_name_for_lucky=input('Enter Name: ')         
                customer_location_for_lucky=input('Enter Location: ')
                print('You have succesfully registered for a lucky draw')
                break
            else:
                print('Enter valid Customer ID')
    def lucky_draw_board(self):
        coupons={'new smartphone','new helmet','cookware','flat 2000 off on a famous shopping site'}
        spin_the_board=int(input('To spin the Lucky Draw board enter 1: '))
        if spin_the_board==1:
            winners_coupon=coupons.pop()
            print(f'Congrats you win {winners_coupon}')
if customer_id not in Customer_details.customer_details:
    print('Need to sign up')
    customer1=luck_draw(customer_id)
    customer1.sign_up()
